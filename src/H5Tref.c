 ...
 */
htri_t
H5T__ref_set_loc(H5T_t *dt, H5T_loc_t loc)
{
    ...
    if (loc != dt->shared->u.atomic.u.r.loc) {
        switch (loc) {
            ...
            case H5T_LOC_DISK: {
                ...
                /* Get container */
                if (NULL == (container = H5VL_get_primary_container()))
                    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTGET, FAIL, "unable to retrieve current container")
                ...
                if (dt->shared->u.atomic.u.r.rtype == H5R_OBJECT1) {
                    ...
                    /* Get container info */
                    if (H5VL_container_get(container, &vol_cb_args) < 0)
 ...
 */
static herr_t
H5T__ref_mem_isnull(const H5VL_container_t H5_ATTR_UNUSED *container, const void *buf, hbool_t *isnull)
{
    static const unsigned char zeros[H5T_REF_MEM_SIZE] = {0};
    ...
    *isnull = (0 == HDmemcmp(buf, zeros, H5T_REF_MEM_SIZE)) ? TRUE : FALSE;
 ...
 */
static herr_t
H5T__ref_mem_setnull(const H5VL_container_t H5_ATTR_UNUSED *container, void *buf, H5_ATTR_UNUSED void *bg_buf)
{
    FUNC_ENTER_STATIC_NOERR
    H5T_REF_LOG_DEBUG("");

    HDmemset(buf, 0, H5T_REF_MEM_SIZE);
 ...
 */
static size_t
H5T__ref_mem_getsize(const H5VL_container_t H5_ATTR_UNUSED *src_container, const void *buf,
                     size_t H5_ATTR_UNUSED ref_size, const H5VL_container_t *dst_container, hbool_t *dst_copy)
{
    ...
    /* Sanity check */
    HDassert(buf);
    HDassert(ref_size == H5T_REF_MEM_SIZE);

    if (NULL != dst_container) {
        ...
        if (H5VL_file_is_same_as_container(vol_obj, dst_container, &files_equal) < 0)
    ...
    if (flags || !ref->encode_size) {
        ...
        if (ref->type == (int8_t)H5R_DATASET_REGION2) {
                  ...
                  size_t H5_ATTR_UNUSED src_size, const H5VL_container_t *dst_container, void *dst_buf, size_t dst_size)
{
    ...
    if (src_ref->type == (int8_t)H5R_DATASET_REGION2) {
        hbool_t is_native = FALSE; /* Whether the dest. container is using the native VOL connector */
                   ...
                   const H5VL_container_t H5_ATTR_UNUSED *dst_container, void *dst_buf, size_t dst_size, void H5_ATTR_UNUSED *bg_buf)
{
    ...
    if (NULL == H5R_REF_FILENAME(&tmp_ref)) {
        ...
        if ((file_id = H5F_reopen_with_container(src_container, H5_REQUEST_NULL)) < 0)
            HGOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, FAIL, "unable to reopen file")
    ...
    if ((file_id != H5I_INVALID_HID) && (H5I_dec_ref(file_id) < 0))
        HDONE_ERROR(H5E_REFERENCE, H5E_CANTDEC, FAIL, "unable to decrement refcount on file ID")
 ...
 */
static herr_t
H5T__ref_disk_isnull(const H5VL_container_t *container, const void *buf, hbool_t *isnull)
{
    ...
    if (ref_type) {
        ...
        *isnull = FALSE;
        ...
        p = (const uint8_t *)buf + H5R_ENCODE_HEADER_SIZE + H5_SIZEOF_UINT32_T;
        ...
        if (H5VL_blob_specific(container, (void *)p, &vol_cb_args) < 0)
 ...
 */
static herr_t
H5T__ref_disk_setnull(const H5VL_container_t *container, void *buf, void *bg_buf)
{
    H5VL_blob_specific_args_t vol_cb_args; /* Arguments to VOL callback */
    ...
    if (p_bg) {
        ...
        if (H5VL_blob_specific(container, (void *)p_bg, &vol_cb_args) < 0)
    ...
    /* Set blob ID to "nil" */
    if (H5VL_blob_specific(container, q, &vol_cb_args) < 0)
 ...
 */
static size_t
H5T__ref_disk_getsize(const H5VL_container_t H5_ATTR_UNUSED *src_container, const void *buf, size_t buf_size,
                      const H5VL_container_t H5_ATTR_UNUSED *dst_container, hbool_t *dst_copy)
{
    const uint8_t *p = (const uint8_t *)buf;
    unsigned       flags;
    H5R_type_t     ref_type;
                   ...
                   const H5VL_container_t H5_ATTR_UNUSED *dst_container, void *dst_buf, size_t dst_size)
{
    ...
    /* Retrieve blob */
    if (H5VL_blob_get(src_container, p, q, blob_size, NULL) < 0)
 ...
 */
static herr_t
H5T__ref_disk_write(H5VL_container_t H5_ATTR_UNUSED *src_container, const void *src_buf, size_t src_size,
                    H5R_type_t H5_ATTR_UNUSED src_type, const H5VL_container_t *dst_container, void *dst_buf,
                    size_t dst_size, void *bg_buf)
{
    const uint8_t *p             = (const uint8_t *)src_buf;
    uint8_t *      q             = (uint8_t *)dst_buf;
    ...
    if (p_bg) {
        ...
        /* Remove blob for old data */
        if (H5VL_blob_specific(dst_container, (void *)p_bg, &vol_cb_args) < 0)
            HGOTO_ERROR(H5E_DATATYPE, H5E_CANTREMOVE, FAIL, "unable to delete blob")
    } /* end if */
    ...
    /* Store blob */
    if (H5VL_blob_put(dst_container, p, src_size, q, NULL) < 0)
 ...
 */
static herr_t
H5T__ref_obj_disk_isnull(const H5VL_container_t *container, const void *buf, hbool_t *isnull)
{
    H5F_t *        f;
    const uint8_t *p = (const uint8_t *)buf;
    haddr_t        addr;
 ...
 */
static herr_t
H5T__ref_obj_disk_read(const H5VL_container_t *src_container, const void *src_buf, size_t src_size,
                       const H5VL_container_t H5_ATTR_UNUSED *dst_container, void *dst_buf,
 ...
 */
static herr_t
H5T__ref_dsetreg_disk_isnull(const H5VL_container_t *container, const void *src_buf, hbool_t *isnull)
{
    H5F_t *        f;
    const uint8_t *p = (const uint8_t *)src_buf;
    haddr_t        addr;
 ...
 */
static herr_t
H5T__ref_dsetreg_disk_read(const H5VL_container_t *src_container, const void *src_buf, size_t src_size,
                           const H5VL_container_t H5_ATTR_UNUSED *dst_container, void *dst_buf,
                           size_t H5_ATTR_UNUSED dst_size)
{
    H5F_t *                src_f;
